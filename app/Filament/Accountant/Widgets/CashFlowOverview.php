...
class CashFlowOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Today\'s Revenue', new \Illuminate\Support\HtmlString('<span style="font-size: 1.25rem; font-weight: 700;">' . number_format($dailyBookings, 2) . ' ' . app(\App\Settings\AppSettings::class)->getIsoCurrency() . '</span>'))
                ->description('Collected today')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->color('success'),
                
            Stat::make('Weekly Revenue', new \Illuminate\Support\HtmlString('<span style="font-size: 1.25rem; font-weight: 700;">' . number_format($weeklyBookings, 2) . ' ' . app(\App\Settings\AppSettings::class)->getIsoCurrency() . '</span>'))
                ->description('Collected this week')
                ->descriptionIcon('heroicon-m-banknotes')
                ->color('primary'),

            Stat::make('Outstanding Balances', new \Illuminate\Support\HtmlString('<span style="font-size: 1.25rem; font-weight: 700;">' . number_format($outstandingBalances, 2) . ' ' . app(\App\Settings\AppSettings::class)->getIsoCurrency() . '</span>'))
