    ...
    @if(empty($links))
        <div style="display:flex;align-items:center;gap:12px;border:1px solid #fcd34d;background:#fffbeb;border-radius:8px;padding:16px;">
            <p style="font-size:14px;color:#92400e;margin:0;">
                ⚠️ No managers or greeters with phone numbers found. Please ensure phone numbers are set on their profiles.
            </p>
        </div>
    @else
        <p style="font-size:14px;color:#6b7280;margin:0 0 12px 0;">
            Click <strong>Send WhatsApp</strong> for each person you want to notify about the
            <strong>{{ \Carbon\Carbon::parse($dispatch->dispatch_date)->format('d/m/Y') }}</strong> balloon dispatch.
            A new browser tab will open with the message pre-filled — just press Send.
        </p>
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;margin-bottom:10px;background:#fff;">
                <div>
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px;">
                        <p style="font-weight:600;font-size:15px;color:#111827;margin:0;">{{ $link['name'] }}</p>
                        @php
                            $roleBg    = $link['role'] === 'manager' ? '#0ea5e9' : '#10b981';
                            $roleLabel = ucfirst($link['role']);
                        @endphp
                        <span style="background:{{ $roleBg }};color:#fff;font-size:11px;font-weight:600;padding:2px 8px;border-radius:999px;">
                            {{ $roleLabel }}
                        </span>
                    </div>
                ...
                <a
                    ...
                    style="display:inline-flex;align-items:center;gap:8px;background:#16a34a;color:#fff;font-size:13px;font-weight:600;padding:8px 16px;border-radius:8px;text-decoration:none;transition:background 0.15s;white-space:nowrap;"
